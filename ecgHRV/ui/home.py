...
# Origem: 12,Junho,2015
# Diego R. Paez Ardila
# Ubicacion: IEB-UFSC - Brasil
# Estatus : Read adc in a thread ->class to not loosing resolution.  
# Update: 12, Junho, 2015
"""
...
# Fin inicializacion del Constructor-------------------------------------------------------------
    ...
    def crearDir(self):
        ...
        carpeta = "ecgHRV/" +  str(self.dataread)
...
class ADCRead(QtCore.QObject):
    ...
    def stop(self):
        self._exit = True
        self.adctimer.stop()
        np.savetxt('ecgRaw' +  '.txt', self.emgReadThread, fmt='%10.4f') # salvar archivo emg
    ...
    def loop(self):
        ...
        self.adctimer.setInterval(4) # Fs= 100Hz = 10ms Fs = 250 Hz = 4ms # Fs = 1000Hz = 1ms
        self.adctimer.start()
